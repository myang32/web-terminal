    ...
    io.sockets.on("connection", function (socket) {
        ...
        function execCmd(command, shell, terminate) {
            ...
            proc.stdout.on("data", function (data) {
                const prompt = data.match(/\015\012(C:\\.*)>$/);
                const out = data.replace(/\015\012C:\\.*>$/, '');
                if (prompt) {
                  cwd = prompt[1];
                ...
                if (undefined === cmd) {
                  // console.log('slurp stdout', data)
                } else {
                  if (cmd === data.trim()) {
                    // console.log('slurp command from stdout', data)
                  } else {
                    socket.emit("console", out);
                  }
                }
            });
        ...
        function begin() {
            ...
            socket.emit("exit");
            execCmd(); // start a cmd shell and slurp welcome text
        }
